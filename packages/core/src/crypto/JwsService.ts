...
export class JwsService {
  private async createJwsBase(agentContext: AgentContext, options: CreateJwsBaseOptions) {
    ...
    if (x5c) {
      const x509Service = await agentContext.dependencyManager.resolve(X509Service)
      const certificate = x509Service.parseCertificate(x5c[0])
      if (
  ...
  private async jwkFromJws(
    ...
    if (protectedHeader.x5c) {
      if (!Array.isArray(protectedHeader.x5c) || typeof protectedHeader.x5c[0] !== 'string') {
        throw new CredoError('X5c is not a valid JSON array of string.')
      }

      const x509Service = await agentContext.dependencyManager.resolve(X509Service)
      const certificate = x509Service.parseCertificate(protectedHeader.x5c[0])
